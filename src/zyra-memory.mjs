...
const MEMORY_DIR = ".zyra/memory";
const LEGACY_MEMORY_DIR = ".cara/memory";
let memoryCache;
...
const LAYERS = [
  ...
  },
  {
    ...
    seed: [
      "# Projects And Tools",
      "",
      "- Zyra CLI is a local Pi SDK based coding agent made for Cara.",
...
export function buildMemoryOverview(root) {
  const layers = ensureZyraMemory(root);
  const lines = ["What Zyra knows about Cara"];
...
export function buildRecommendedPrompts(root, limit = 1) {
  const layer = ensureZyraMemory(root).find((item) => item.file === "recommended-prompts.md");
  if (!layer) return [];
...
export function buildConsolidationPrompt(runtime, globalAgentFiles = []) {
  ...
  return `Consolidate Zyra memory about Cara now.
...
function getLegacyMemoryDir(root) {
  return path.join(root, LEGACY_MEMORY_DIR);
}

function migrateLegacyMemory(root) {
  const dir = getMemoryDir(root);
  const legacy = getLegacyMemoryDir(root);
